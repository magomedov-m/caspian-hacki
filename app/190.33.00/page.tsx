...
export default function AdminPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      // Проставляем "IN_PROGRESS" по умолчанию, если статус не задан
      const normalized = data.map(r => ({
        ...r,
        status: r.status === "IN_PROGRESS" || r.status === "DONE" ? r.status : "IN_PROGRESS"
      }));
      ...
      return (
        <>
          ...
          <div className="flex gap-3 mb-4">
            <button
              className={`px-4 py-2 rounded-lg text-sm border transition ${filter === "all" ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"}`}
              onClick={() => setFilter("all")}
            >
              Все
            </button>
            <button
              className={`px-4 py-2 rounded-lg text-sm border transition ${filter === "IN_PROGRESS" ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"}`}
              onClick={() => setFilter("IN_PROGRESS")}
            >
              В процессе
            </button>
            <button
              className={`px-4 py-2 rounded-lg text-sm border transition ${filter === "DONE" ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"}`}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredReports.map(report => (
              <div key={report.id} className="bg-white border rounded-xl p-5 shadow-sm hover:shadow-md flex flex-col justify-between">
    ...
    if (activeSection === "users") {
      return (
        <>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map(user => (
              <div key={user.id} className="bg-white border rounded-xl p-5 shadow-sm hover:shadow-md w-90">
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      <aside className="w-full md:w-64 bg-white border-r shadow-sm p-6 flex flex-col gap-6 mt-12">
        ...
        <Link href='/'>
          <Button variant="outlined" className="mt-auto bg-blue-600 text-white rounded-lg py-2 hover:bg-blue-700 transition">
            На главную
          </Button>
        </Link>
      </aside>
